...
/**
 * Main interface for Exonum Light client. Provides a convenient way for interaction with Exonum
 * framework APIs. All the methods of the interface work in a blocking way, i.e., invoke underlying
 * request immediately, and block until the response can be processed or an error occurs. In case
 * the thread is interrupted, the blocked methods will complete exceptionally.
 *
...
public interface ExonumClient {
   ...
   * @throws RuntimeException if the client is unable to submit the transaction (e.g., in case of
   *     connectivity problems)
   */
  HashCode submitTransaction(TransactionMessage tx);

  /**
   * Returns a number of unconfirmed transactions which are currently located in the unconfirmed
   * transactions pool and are waiting for acceptance to a block.
   *
   * @throws RuntimeException if the client is unable to complete a request (e.g., in case of
   *     connectivity problems)
   */
  ...
  /**
   * Returns the information about the transaction; or {@code Optional.empty()} if the requested
   ...
   * @throws IllegalArgumentException if the given height is negative; or greater than the actual
   *     blockchain height
   * @throws RuntimeException if the client is unable to complete a request (e.g., in case of
   *     connectivity problems)
   */
  ...
  /**
   * Returns blockchain blocks in the requested <em>closed</em> range. The blocks are returned in
   * ascending order by their height.
   *
   * @param fromHeight the height of the first block to include. Must be non-negative
   * @param toHeight the height of the last block to include. Must be greater than or equal to
   *     {@code fromHeight} and less than or equal to the blockchain height.
   * @param blockFilter controls whether to skip blocks with no transactions
   * @param timeOption controls whether to include the {@linkplain Block#getCommitTime() block
   *     commit time}
   * @return blocks in the requested range
   * @throws IllegalArgumentException if {@code fromHeight} or {@code toHeight} are not valid: out
   *     of range {@code [0, blockchainHeight]}; {@code fromHeight} > {@code toHeight}
   * @throws RuntimeException if the client is unable to complete a request (e.g., in case of
   *     connectivity problems)
   */
  List<Block> getBlocks(
      long fromHeight, long toHeight, BlockFilteringOption blockFilter, BlockTimeOption timeOption);

  /**
   * Returns the range of the most recent blockchain blocks in ascending order by their height. More
   * precisely, returns the blocks in the closed range {@code [max(0, blockchainHeight - size + 1),
   * blockchainHeight]} of size {@code max(blockchainHeight + 1, size)}.
   *
   * @param size the size of the range. If it exceeds the number of blocks in the blockchain, this
   *     method will return all blocks ({@code blockchainHeight + 1})
   * @param blockFilter controls whether to skip blocks with no transactions. If filtering is
   *     applied, the actual number of blocks may be smaller than {@code size}; but the range of
   *     blocks will not be extended beyond {@code blockchainHeight - size + 1}. If a certain
   *     <em>number</em> of non-empty blocks is needed (not a certain <em>range</em>), use {@link
  ...
  /**
   * Returns up to the given number of the most recent non-empty blocks in ascending order by their
   * height. The search range is not limited, i.e., spans the whole blockchain.
   *
   * @param numBlocks the maximum number of blocks to return. Must be positive. If the number of
   *     non-empty blocks in the blockchain is less than {@code numBlocks}, all such blocks will be
   *     returned
   * @param timeOption controls whether to include the {@linkplain Block#getCommitTime() block
   *     commit time}
  ...
  /**
   * Returns the last block in the blockchain which contains transactions; or {@code
   * Optional.empty()} if there are no blocks with transactions in the blockchain.
   *
   * @throws RuntimeException if the client is unable to complete a request (e.g., in case of
   *     connectivity problems)
   */
  ...
  /**
   * Returns the service info of a started service instance with a given name; or {@code
   * Optional.empty()} if there is no service instance with such name.
   *
  ...
  class Builder {
     ...
     * @see <a href="https://exonum.com/doc/version/latest/architecture/configuration/#api">API
     *     Configuration</a>
     * @see <a
     *     href="https://exonum.com/doc/version/latest/get-started/java-binding/#node-configuration">
     ...
     *
     * <p>This method provides a flexibility for the Exonum client configuration. Can be helpful,
     * for example, in case a network proxy configuration is needed or <a
    ...
    /**
     * Sets an optional URL prefix to be applied to all requests made by the client. Can be helpful
     * in case of using middleware routing proxy on the blockchain node side. There is no prefix by
     * default.
     */
