...
pub fn create_vm(debug: bool) -> JavaVM {
    let (classpath, libpath) = get_classpath_option().unwrap();
...
fn get_classpath_option() -> io::Result<(String, String)> {
    ...
    let path = Path::new("../../../exonum-java-binding-fakes/target/ejb-fakes-classpath.txt")
        .canonicalize()?;
    ...
    let library_path = Path::new("../../rust/target/debug")
        .canonicalize()
