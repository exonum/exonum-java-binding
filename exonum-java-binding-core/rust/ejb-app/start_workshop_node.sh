...
EJB_CLASSPATH="${EJB_CLASSPATH}:${EJB_ROOT}/../java-sga-workshop/target/classes"
...
RUST_LIB_DIR="$(rustup run nightly rustc --print sysroot)/lib"
...
cargo run -- finalize testnet/sec.toml testnet/node.toml \
 --ejb-module-name 'com.exonum.workshop.ServiceModule' \
 --ejb-port 6000 \
