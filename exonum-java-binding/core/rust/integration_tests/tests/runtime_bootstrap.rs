...
use java_bindings::{
    create_java_vm, create_service_runtime, Executor, InternalConfig, JavaRuntimeProxy, JvmConfig, RuntimeConfig,
};
...
fn bootstrap() {
    ...
    let runtime: (u32, Box<dyn Runtime>) = create_service_runtime(executor, &runtime_config)
        .into();

    assert_eq!(runtime.0, JavaRuntimeProxy::RUNTIME_ID as u32);
}
