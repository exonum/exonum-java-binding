...
/**
 * A loader of the native shared library with Exonum framework bindings. It loads the native library
 * and also verifies that it is compatible with the Java classes. The native library is compatible
 * iff it has exactly the same version as this Java library. The revision from which they were built
 * is currently not checked, but may be in the future (see ECR-3173), because the API between Java
 * and native is considered internal and can be changed in an incompatible way in any revision.
 *
 * <p>To enable loading of java_bindings library, add a path to the folder containing it to <code>
 * java.library.path</code> property, e.g.: <code>
 * java -Djava.library.path=${EXONUM_HOME}/lib/native …</code>
 *
 ...
 * @see <a href="https://exonum.com/doc/version/0.13-rc.2/get-started/java-binding/#testing">Build
 *     configuration to enable integration testing of Java services</a>
 */
public final class LibraryLoader {
  ...
  /**
   * The current version of the project. Must be updated on <a
   * href="https://wiki.bf.local/display/EJB/Java+Binding+Release+Checklist+Template">each
   * release</a>.
   */
   ...
   *
   * @param libraryVersion the version of this library to verify that the native library is
   *     compatible with it
   */
   ...
   *
   * @throws LinkageError if the native library cannot be loaded; or if it is incompatible with this
   *     library version
   */
  ...
  private static void loadLibrary() {
    try {
      ...
      String message =
          String.format(
              "Failed to load '%s' library: %s.%n%s",
              BINDING_LIB_NAME, e.getMessage(), extraLibLoadErrorInfo());
  ...
  private static String extraLibLoadErrorInfo() {
    ...
    if (runningUnitTests()) {
      ...
      return String.format(
          "Unknown error: please submit an issue including this error message.%n"
              + "%s=%s%n"
              + "%s=%s",
          JAVA_LIBRARY_PATH_PROPERTY, javaLibPath, dynamicLibVar, dynamicLibValue);
    }
  }
