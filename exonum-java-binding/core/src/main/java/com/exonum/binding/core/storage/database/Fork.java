 ...
 *
 * <p>A fork represents the database state at the time it was created <em>plus</em> any changes to
 * the database made using this fork.
 *
 * <p>A fork allows to perform a transaction: a number of independent writes to the database, which
 * then may be <em>atomically</em> applied (i.e. committed) to the database and change the database
 * state.
 */
public final class Fork extends AbstractAccess {

  /**
   * A destructor of the native fork object. This class keeps a destructor to be able to cancel it
   * on peer ownership transfer, happening in {@link #intoPatch()}.
   */
   ...
   * the objects depending on the fork, primarily — indexes, when it is converted into patch and
   * invalidated or rolled-back (which requires collection invalidation).
   *
   * <p>It is a "child" of the {@link #forkCleaner} which destroys the fork itself and, through this
   * cleaner, any dependent objects.
   */
  ...
  /**
   * Converts this fork into a patch that can be merged into the database. This method will close
   * any resources registered with {@linkplain #getCleaner() its cleaner} (typically, indexes,
   * iterators), convert this fork into a patch, invalidate this fork, and return a handle to the
   * patch. The caller is responsible for properly destroying the returned patch.
   *
   * <p>Subsequent operations with the fork are prohibited.
   *
   * @return a handle to the patch obtained from this fork
   * @see <a href="https://exonum.com/doc/version/0.13-rc.2/architecture/merkledb/#patches">MerkleDB
   *     Patches</a>
   */
  NativeHandle intoPatch() {
    checkState(
        nativeCanConvertIntoPatch(getNativeHandle()), "This fork cannot be converted into patch");
    ...
    try {
      ...
      throw new IllegalStateException(
          "intoPatch aborted because some dependent resources " + "did not close properly", e);
    }
  ...
  public void createCheckpoint() {
    ...
    checkState(nativeCanRollback(getNativeHandle()), "This fork does not support checkpoints");
   ...
   * changes made by this fork. Rollback affects only changes made with this particular Fork
   * instance.
   *
  ...
  public void rollback() {
    checkState(nativeCanRollback(getNativeHandle()), "This fork does not support rollbacks");
  ...
  /**
   * Converts this fork into patch, consuming the object, and returns the native handle to the
   * patch.
   *
   * <p>In case of failure RuntimeException is thrown and provided nativeHandle is invalidated.
   */
  private static native long nativeIntoPatch(long nativeHandle);

  /**
   * Returns true if creating checkpoints and performing rollbacks is possible with this particular
   * Fork instance.
   *
  ...
  /**
   * Rollback changes to the latest checkpoint. Affects only changes made with this particular Fork
   * instance.
   */
