...
public abstract class Block {
   ...
   * block. Genesis block has 0 height. Therefore, the blockchain height is equal to the number of
   * blocks plus one.
   *
  ...
  /**
   * Returns true if this block is empty: contains no {@linkplain #getNumTransactions()
   * transactions}.
   */
  ...
  /**
   * Root hash of the Merkle tree of transactions in this block. These transactions can be accessed
   * with {@link Blockchain#getBlockTransactions(Block)}.
   */
  ...
  /**
   * Contains additional block headers of the block. The key is a block header; and the value is a
   * header bytes value.
   */
   ...
   * @throws IllegalArgumentException if the block bytes are not a serialized {@link
   *     com.exonum.core.messages.Blockchain.Block}
   */
  ...
  public abstract static class Builder {
    ...
    /**
     * Sets the block height, which is the distance between the block and the genesis block, which
     * has zero height. Must be non-negative.
     */
    ...
    /**
     * Sets the hash of the previous block in the hash chain. The previous block is the block with
     * the height, that is equal to this minus one. Genesis block has a previous hash of zeroes.
     */
    public abstract Builder previousBlockHash(HashCode previousBlockHash);

    /**
     * Sets the Merkle root hash of the collection holding all transactions in this block. This
     * collection and transactions can be accessed with {@link
     * Blockchain#getBlockTransactions(Block)}.
     */
    public abstract Builder txRootHash(HashCode txRootHash);

    // TODO: Expand on that when it has meaningful applications: ECR-2756
    /**
     * Sets the blockchain state hash at the moment this block was committed. The blockchain state
     * hash reflects the state of each service in the database.
     *
     * @see Schema
     */
    ...
    /**
     * Sets additional block headers. Headers should have exactly the same order as its native
     * equivalent. Changing headers order will lead to the block hash violation.
     */
    ...
    public Block build() {
      ...
      checkState(
          block.getNumTransactions() >= 0,
          "numTransaction is negative: %s",
