...
class ApiControllerTest {
  ...
  private static final Gson JSON_SERIALIZER =
      JsonSerializer.builder().registerTypeAdapterFactory(CoreTypeAdapterFactory.create()).create();

  @Mock QaService qaService;
  ...
  void submitIncrementCounter(VertxTestContext context) {
    ...
    MultiMap params = multiMap("seed", Long.toString(seed), "counterId", String.valueOf(counterId));

    when(qaService.submitIncrementCounter(eq(seed), eq(counterId))).thenReturn(EXPECTED_TX_HASH);
  ...
  void submitUnknown(VertxTestContext context) {
    when(qaService.submitUnknownTx()).thenReturn(EXPECTED_TX_HASH);

    post(SUBMIT_UNKNOWN_TX_PATH).send(checkCreatedTransaction(context, EXPECTED_TX_HASH));
  }
  ...
  void getCounter(VertxTestContext context) {
    ...
    when(qaService.getValue(eq(id))).thenReturn(Optional.of(counter));
  ...
  void getCounter_NoCounter(VertxTestContext context) {
    HashCode id = HASH_1;
    when(qaService.getValue(id)).thenReturn(Optional.empty());
  ...
  void failureHandlerUnexpectedException(VertxTestContext context) {
    ...
    // RuntimeException.
    when(qaService.getValue(id)).thenThrow(new RuntimeException(message));
    String getCounterUri = getCounterUri(id);
  ...
  @Test
  void multiMapTest() {
    MultiMap m = multiMap("k1", "v1", "k2", "v2", "k3", "v3");
                ...
                response ->
                    context.verify(
                        () -> {
                          ...
                          String body = response.bodyAsString();
                          TimeDto actualTime = JSON_SERIALIZER.fromJson(body, TimeDto.class);
    ...
    Map<PublicKey, ZonedDateTime> validatorsTimes =
        ImmutableMap.of(
            ...
            PublicKey.fromHexString("22"),
                ZonedDateTime.of(2018, 1, 1, 0, 0, 1, 0, ZoneOffset.UTC));
                ...
                response ->
                    context.verify(
                        () -> {
                          ...
                          String body = response.bodyAsString();
                          Map<PublicKey, ZonedDateTime> actualValidatorsTimes =
                              JSON_SERIALIZER.fromJson(
                                  body,
                                  new TypeToken<Map<PublicKey, ZonedDateTime>>() {}.getType());
  ...
  static MultiMap multiMap(String k1, String v1, String... entries) {
    ...
    MultiMap params = MultiMap.caseInsensitiveMultiMap().add(k1, v1);
    int numEntries = entries.length / 2;
    IntStream.range(0, numEntries).forEach(i -> params.add(entries[2 * i], entries[2 * i + 1]));
