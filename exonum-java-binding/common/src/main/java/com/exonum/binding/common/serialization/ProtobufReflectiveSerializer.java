 ...
 *
 * <p>Uses {@linkplain CodedOutputStream#useDeterministicSerialization() deterministic} protocol
...
class ProtobufReflectiveSerializer<MessageT extends MessageLite> implements Serializer<MessageT> {
  ...
  ProtobufReflectiveSerializer(Class<MessageT> messageType) {
    ...
    try {
      messageParseFrom =
          lookup.findStatic(
              messageType, "parseFrom", MethodType.methodType(messageType, byte[].class));
    } catch (NoSuchMethodException | IllegalAccessException e) {
      throw new IllegalArgumentException(
          "Invalid message: cannot find public static parseFrom " + "method in " + messageType, e);
    }
  }
  ...
  public byte[] toBytes(MessageT value) {
    ...
    try {
      ...
      throw new AssertionError(
          "Failed to serialize " + value + " to a byte array (should never happen)", e);
    }
  }
