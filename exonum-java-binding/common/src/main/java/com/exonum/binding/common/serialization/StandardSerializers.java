...
public final class StandardSerializers {
  ...
  /**
   * Returns a serializer of integers as four bytes in little-endian byte order. More efficient than
   * {@link StandardSerializers#uint32()} if values are often greater than {@code 2^28}.
   */
  ...
  /**
   * Returns a serializer of unsigned integers using variable length encoding. These more
   * efficiently encodes values for the range {@code [0; 2^21-1]} than {@link
  ...
  /**
   * Returns a serializer of signed integers using variable length encoding. These more efficiently
   * encodes values for the range {@code [-2^20; 2^20-1]} than {@link
   * StandardSerializers#fixed32()}. If your values are strictly non-negative, consider using {@link
  ...
  /**
   * Returns a serializer of longs as eight bytes in little-endian byte order. More efficient than
   * {@link StandardSerializers#uint32()} if values are often greater than {@code 2^56}.
   */
  ...
  /**
   * Returns a serializer of unsigned longs using variable length encoding. These more efficiently
   * encodes values for the range {@code [0; 2^49-1]} than {@link StandardSerializers#fixed64()}.
   */
  ...
  /**
   * Returns a serializer of signed longs using variable length encoding. These more efficiently
   * encodes values for the range {@code [-2^48; 2^48-1]} than {@link
  ...
  /**
   * Returns a serializer of strings in UTF-8. Deserializer will reject malformed input, but replace
   * the characters not representable in UTF-16 with the default replacement character.
   */
  ...
  /**
   * Returns a serializer for the given protocol buffer message type. The returned serializer uses
   * {@linkplain CodedOutputStream#useDeterministicSerialization() deterministic} serialization
   * mode.
   *
   * @param messageType the class of a protobuf message
   * @param <MessageT> the type of a message; must have a public static {@code #parseFrom(byte[])}
   *     method — as any auto-generated protobuf message does
   * @throws IllegalArgumentException if {@code MessageT} does not contain the static factory method
   *     {@code #parseFrom(byte[])}
   */
